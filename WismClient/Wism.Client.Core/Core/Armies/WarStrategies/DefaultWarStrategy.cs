    ...
    public class DefaultWarStrategy : IWarStrategy
    {
        ...
        public bool Attack(List<Army> attackers, Tile tile)
        {
            ...
            while (attackers.Count > 0 && defenders.Count > 0)
            {
                ...
                // Refresh the list
                defenders = attackers.Count > 0
                    ? GetDefenders(attackers, tile)
                    : new List<Army>();
            }
            ...
            out List<Army> defenders, out int compositeAFCM, out int compositeDFCM)
        {
            ...
            // Neutral cities already defend with a generated neutral garrison whose strength is
            // equal to the city defense rating, so applying the city bonus again would double-count.
